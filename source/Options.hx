...
class OldAssets extends Option
{
	...
	{
		FlxG.save.data.old = !FlxG.save.data.old;
		display = updateDisplay();
		return true;
	}
	...
	{
		return (!FlxG.save.data.old ? "new assets" : "old assets");
	}
}
...
class ScreenShake extends Option
{
	...
	public override function press():Bool
	{
		FlxG.save.data.shakey = !FlxG.save.data.shakey;
		display = updateDisplay();
		return true;
	}
	...
	private override function updateDisplay():String
	{
		return "Screen Shake " + (!FlxG.save.data.shakey ? "off" : "on");
	}
}
